...
main (int argc, char **argv)
{
  ...
  struct pbook_s *already_inserted;
  HTABLE_HEAD (pbook, HSIZE, pbook_s) my_table = HTABLE_INITIALIZER (&my_table);
  
  printf ("***********************************\n");
  printf ("* Hash Table multiple insert test *\n");
  printf ("***********************************\n");
  ...
  already_inserted = HTABLE_INSERT (pbook, &my_table, &my_book[0]);
  printf ("Is the pointer to newly inserted element nil (should be YES) ? "
          "%s\n", (already_inserted == NULL) ? "YES" : "NO");
