...
main (int argc, char **argv)
{
  ...
  find.name = my_book[0].name;
  res = HTABLE_LOOKUP (pbook, &my_table, &find);

  printf ("Phone number of \"%s\": \"%s\" (should be \"%s\")\n",
          find.name, res->phone, my_book[0].phone);
